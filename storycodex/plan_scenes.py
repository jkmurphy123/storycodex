...
def build_prompt(
    ...
    instruction = (
        "Generate scene index and scene plans JSON. Return JSON only, no extra text, no markdown fences. "
        "Use the spine scene IDs exactly; scenes are global sequential integers. "
        "The index must include all scenes from the spine, with plan_path and beats_path "
        "matching artifacts/scenes/scene_###.plan.json and .beats.json. "
        "For each plan, include title, setting, goal, stakes, cast (0-4 entries), and beats_ref matching the beats_path. "
        "Use short slug-like location_id values. Keep content concise. "
        f"{chapter_line}\n\n"
        "Output format:\n"
