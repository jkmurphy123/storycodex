...
def write_diary(
    ...
    output_path = root / "out" / "diary" / f"scene_{scene_id:03d}.draft.md"
    ...
    prompt = build_diary_prompt(context, target, length)
    ...
    if not ok:
        retry_prompt = build_diary_retry_prompt(context, target, length, issues)
        draft = llm.chat(retry_prompt, chosen_model, temperature=0.7, max_tokens=max_tokens)
        ok, issues = validate_draft(draft, target, context)
    if not ok:
        expand_prompt = build_diary_expand_prompt(context, target, draft)
    ...
    meta_path = root / "out" / "diary" / f"scene_{scene_id:03d}.draft.meta.json"
...
def build_diary_prompt(
    context: dict[str, Any], target_words: int, length: str
) -> list[dict[str, str]]:
    context_json = json.dumps(context, indent=2, sort_keys=True)
    rules = (
        "Hard rules:\n"
        "- Use ONLY the provided context packet.\n"
        "- Write as a diary entry recorded after the day's events.\n"
        "- First-person past tense is mandatory throughout.\n"
        "- Follow all ringA.style_rules (filtered through the diarist's voice).\n"
        ...
        "- If cast size > 1, include dialogue (quoted or paraphrased as remembered).\n"
        "- If cast size == 1, interiority is allowed.\n"
        "- Optional: include a dateline only if the context packet provides a date/day marker.\n"
        "- No meta commentary about being an AI or writing a diary.\n"
    ...
    return [
        {
            "role": "system",
            "content": "You are a professional fiction writer writing a first-person diary entry under strict constraints.",
        },
        {"role": "user", "content": user_content},
    ]


def build_diary_retry_prompt(
    ...
    user_content = (
        ...
        "Rewrite as a diary entry in first-person past tense.\n"
        "Follow ringA.style_rules, obey continuity_locks, and hit beats in order.\n"
        f"Issues:\n{issue_text}\n\n"
...
def build_diary_expand_prompt(
    ...
    user_content = (
        ...
        "Keep first-person past tense diary voice, preserve beat order, add detail and dialogue where appropriate. "
        f"Target length: {min_words}-{max_words} words. "
        "Return the FULL expanded diary entry, no commentary.\n\n"
        "Context packet JSON:\n"
        f"{context_json}\n\n"
        "Draft to expand:\n"
        f"{draft}"
